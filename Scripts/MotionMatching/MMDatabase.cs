...
public class MMDatabase 
{
    ...
    /* public void ComputeFeaturesOld()
    {
       ...
       nFeatures =
        ...
        if(settings.databaseType == MMDatabaseType.Trajectory) { 
         ...
         nFeatures += 6 + // Trajectory Positions 2D
        ...
        featuresScale = new float[nFeatures];
        int offset = 0;
        ComputeBonePositionFeature(ref offset, (int)Bones.LeftFoot, settings.weight_foot_position);
        ComputeBonePositionFeature(ref offset, (int)Bones.RightFoot, settings.weight_foot_position);
        ComputeBoneVelocityFeature(ref offset, (int)Bones.LeftFoot, settings.weight_foot_velocity);
        ComputeBoneVelocityFeature(ref offset, (int)Bones.RightFoot, settings.weight_foot_velocity);
        ComputeBoneVelocityFeature(ref offset, (int)Bones.Hips, settings.weight_hip_velocity);
        if (settings.databaseType == MMDatabaseType.Trajectory)
        {
            ComputeTrajectoryPositionFeature(ref offset, 0, settings.weight_trajectory_positions);
            ComputeTrajectoryDirectionFeature(ref offset, 0, settings.weight_trajectory_directions);
        }
    }*/
    ...
    public void Load(string fileName, bool loadMusicFeatures=false)
    {

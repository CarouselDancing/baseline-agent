...
public class RESTInterface : MonoBehaviour
{
    ...
    /// <returns></returns>
    public IEnumerator sendRequestCoroutine(string method, string messageBody, PostRequestCallback callback)
    {
        var data = System.Text.Encoding.UTF8.GetBytes(messageBody);
        string urlString = getMethodURL(method);
        Debug.Log("url"+urlString);
        Debug.Log("messageBody"+messageBody);
    ...
    /// <param name="callback">Callback handler that process the response string. </param>
    /// <returns></returns>
    public IEnumerator sendRequestCoroutine(string method, string messageBody, BytePostRequestCallback callback)
    {
        var data = System.Text.Encoding.UTF8.GetBytes(messageBody);
        string urlString = getMethodURL(method);
        Debug.Log("url"+urlString);
    ...
    /// <param name="callback">Callback handler that process the response string. </param>
    /// <returns></returns>
    public IEnumerator sendGETRequestCoroutine(string method, System.Action<string> callback)
    {
